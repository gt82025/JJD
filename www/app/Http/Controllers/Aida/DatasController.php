    ...
    public function pageData($model, $id = null)
    {
        ...
        switch ($model) {
            case 'Product':
                ...
                if($id){
                    $data = $data::with(['category','spec' => function($query){
                        $query->with(['orders' =>function($query){
                            $query->with('order')->orderBy('order_date','desc');
                        }])->orderBy('id','asc');
                    },'orders' => function($query){
                        $query->with('spec','order')->orderBy('order_date','desc');
                    }])->find($id);
                    ...
                    foreach($picks as $k => $v){
                        $picks[$k]['text'] = $v->name.'-'.$v->taste;
                        $insides[$k]['text'] = $v->name.'-'.$v->taste;
            ...
            case 'Order':
               //$ships = \App\Model\Ship::all();
                ...
                if($id){
                    $data = $data::with(['ship','category','detail' => function($query){
                        $query->with('spec','product')->get();
                    }])->find($id);
                    if(!isset($data))return '';
        ...
        foreach ($datas as $k => $v) {
            ...
            foreach ($column as $k2 => $v2) {
                ...
                switch ($v2['field']) {
                    ...
                    case 'reply': case 'in_menu': case 'in_header':
